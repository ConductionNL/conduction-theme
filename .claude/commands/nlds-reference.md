...
| `<category>`     | `municipalities`                                    | Category folder: `municipalities`, `partnerships`, `water-authorities` or `other` |
| `<folderName>`   | `<category>/den-haag-design-tokens`                 | Directory path                                     |
...
> **Allowed references:** the organisation's website, its huisstijlhandboek, and the `conduction-design-tokens/` baseline files. **Never look at other themes in the category folders (`municipalities/`, `partnerships/`, `water-authorities/`, `other/`) for reference** — they contain organisation-specific choices and mistakes that silently leak into the new theme (e.g. copying another theme's decision to map the body font-size to `font-size.md` instead of the website's actual `<p>` font-size).
...
Create the full directory structure at `<category>/<slug>-design-tokens/` modeled on `conduction-design-tokens/`. Create every file listed below.

> **Do not create or copy the `dist/` folder.** This folder is generated by `npm run build` and will always be overwritten.

---

### `<category>/<slug>-design-tokens/package.json`
...
### `<category>/<slug>-design-tokens/style-dictionary.config.js`
...
### `<category>/<slug>-design-tokens/src/config.json`
...
### `<category>/<slug>-design-tokens/src/index.scss`
...
### `<category>/<slug>-design-tokens/src/font/` — Download font files

For every font file URL found in Part A (`.woff2`, `.woff`, `.ttf`, `.otf`):

1. Download the file using the Bash tool (`curl -L -o ...`) into `<category>/<slug>-design-tokens/src/font/`.
...
### `<category>/<slug>-design-tokens/src/font.scss`
...
#### `<category>/<slug>-design-tokens/src/brand/<slug>/color.tokens.json`
...
#### `<category>/<slug>-design-tokens/src/brand/<slug>/root.tokens.json`
...
#### `<category>/<slug>-design-tokens/src/brand/<slug>/font-size.tokens.json`
...
#### `<category>/<slug>-design-tokens/src/brand/<slug>/size.tokens.json`
...
#### `<category>/<slug>-design-tokens/src/brand/<slug>/typography.tokens.json`
...
#### `<category>/<slug>-design-tokens/src/common/utrecht/action.tokens.json`
...
#### `<category>/<slug>-design-tokens/src/common/utrecht/space.tokens.json`
...
3. Apply only these text substitutions to the copied content:
   - `conduction` → `<slug>`
   - `aldritch` → `<fontKey>`
4. **Write** the result to the equivalent path under `<category>/<slug>-design-tokens/src/` using the Write tool.
...
**`src/component/conduction/`** — these are Conduction framework components. The destination folder is **always** `<category>/<slug>-design-tokens/src/component/conduction/` — **never rename this folder to `<slug>`**.
...
#### `<category>/<slug>-design-tokens/documentation/color.stories.mdx`
...
#### `<category>/<slug>-design-tokens/documentation/components.stories.mdx`
...
### `<category>/<slug>-design-tokens/README.md`
...
```sh
cd <category>/<slug>-design-tokens && npm run build
```
...
3. Install the new package into the template repo (or copy the built dist files from `<category>/<slug>-design-tokens/dist/`).
...
6. Remind the user to:
   ...
   - Add a build script entry in the root `package.json`: `"build:<slug>": "cd ./<category>/<slug>-design-tokens && npm run build"`
